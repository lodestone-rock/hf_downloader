...
def get_hf_repo_url(repo_id: str) -> list:
    ...
    for path in flat_paths:
        if repo_id.split("/")[0] == "datasets":
            folder = "/".join(path.split("/")[2:-1])
        else:
            folder = "/".join(path.split("/")[1:-1])
        urls.append([format + path.replace(repo_id + "/", ""), folder])
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="download",
        choices=["download", "dump"],
        help='Choose "download" to execute wget commands or "dump_command" to dump wget commands to a txt file.',
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=2,
        help="Number of worker threads for concurrency.",
    )
    parser.add_argument(
        "--auth_token", type=str, help="Number of worker threads for concurrency."
    )
    ...
    try:
        if args.flag == "download":
            with ThreadPoolExecutor(max_workers=args.workers) as executor:
                futures = [
                    executor.submit(execute_wget_command, cmd) for cmd in wget_cmd
                ]
